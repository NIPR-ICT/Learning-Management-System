...
<div class="page-wrapper">
    <div class="page-content">
        ...
        <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
            ...
            <div class="ms-auto">
                <div class="btn-group">
                    <a href="{{ route('all.charges') }}" class="btn btn-primary">List of Charges</a>
                </div>
            </div>
        </div>
        <!--end breadcrumb-->
        <h6 class="mb-0 text-uppercase">Edit Charge</h6>
        <hr />
        <div class="card">
            <div class="card-body">
                ...
                <form action="{{ route('charges.update', $charge->id) }}" method="POST">
                    ...
                    <input class="form-control w-100 form-control-lg mb-3" placeholder="E.g ICT Levy" type="text"
                        id="item" name="item" value="{{ old('item', $charge->item) }}" aria-label=".form-control-lg example">
                    <x-input-error :messages="$errors->get('item')" class="mt-2" />

                    <label for="part_id" disabled  class="form-label">Choose the program to which the fee applies.</label>
                    <select class="form-control w-100 form-control-lg mb-3" id="part_id" name="part_id" rea required
                        ...
                        @foreach ($parts as $part)
                            <option value="{{ $part->id }}" data-program="{{ $part->program->title }}"
                                ...
                                {{ $part->id == $charge->part_id ? 'selected' : '' }}>
                    ...
                    <input class="form-control w-100 form-control-lg mb-3" type="number" min="10" id="amount"
                        name="amount" value="{{ old('amount', $charge->amount) }}" aria-label=".form-control-lg example">
                    <x-input-error :messages="$errors->get('amount')" class="mt-2" />

                    <label for="description" class="form-label">Charges Description</label>
                    <textarea class="form-control w-100 form-control-lg mb-3" id="description" name="description" rows="4"
                        style="resize: none" aria-label=".form-control-lg example">{{ old('description', $charge->description) }}</textarea>
